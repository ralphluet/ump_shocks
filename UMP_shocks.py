############################################################################################################
# Obtain Unconventional Monetary Policy Shocks for 2009M1 - 2015M12 period
############################################################################################################
...
RR_ZLB = RR_ZLB.set_index('MTGDATE')

# Create a formula string that specifies the regression model
formula = 'Rate_change ~ ' + ' + '.join([col for col in RR_ZLB.columns if col != 'Rate_change' and col != 'GBdate'])

# Fit the regression model using the formula and the RR_Data DataFrame
model = smf.ols(formula=formula, data=RR_ZLB).fit()

# Get the residuals from the regression
UMP_shocks = model.resid

# Resample the Series by month and fill missing values with 0
UMP_shocks = UMP_shocks.resample('M').sum().fillna(0)
...
UMP_shocks.index = UMP_shocks.index.to_period('M').to_timestamp()

# Save the 2009-2015 shocks
UMP_shocks = UMP_shocks.to_frame('RESID')
UMP_shocks.to_csv('output/monthly/UMP_shocks.csv')

############################################################################################################
# Transform into quarterly shocks
############################################################################################################

UMP_shocks_Q = UMP_shocks.resample('Q').sum().fillna(0)
UMP_shocks_Q.index = UMP_shocks_Q.index.to_period('Q').to_timestamp()

UMP_shocks_Q.to_csv('output/quarterly/UMP_shocks.csv')