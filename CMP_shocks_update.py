############################################################################################################
# Update Romer and Romer (2004) shocks for the period 1969-2008
############################################################################################################
...
RR_6908 = RR_data[(FFR_data['MTGDATE'] >= start) & (RR_data['MTGDATE'] <= end)]
RR_6908 = RR_6908.set_index('MTGDATE')

# Create a formula string that specifies the regression model
formula = 'Rate_change ~ ' + ' + '.join([col for col in RR_6908.columns if col != 'Rate_change' and col != 'GBdate'])

# Fit the regression model using the formula and the RR_Data DataFrame
model = smf.ols(formula=formula, data=RR_6908).fit()
...
shocks.to_csv('output/monthly/CMP_shocks.csv')
...
shocks_Q.to_csv('output/quarterly/CMP_shocks.csv')